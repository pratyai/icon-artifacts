...
inline std::vector<std::string_view> split(std::string_view s, char delim) {
  ...
  for (size_t start_pos = 0, next_pos; start_pos < s.length();
       start_pos = next_pos + 1) {
    next_pos = s.find(delim, start_pos);
    if (next_pos == std::string_view::npos) {
      parts.push_back(s.substr(start_pos));
      break;
    }
    parts.push_back(s.substr(start_pos, next_pos - start_pos));
  }
  return parts;
}
...
inline std::string
read_line(std::istream &s,
          const std::optional<std::string> &should_contain = {}) {
  if (s.eof())
    return "<eof>";
  scroll_space(s);
  char bin[1024]; // Increased buffer size for safety
  s.getline(bin, 1023);
  assert(s.good() || s.eof());
  if (should_contain) {
    if (std::string_view(bin).find(*should_contain) == std::string_view::npos) {
...
inline std::map<void *, array_meta> &ARRAY_META_DICT() {
  static std::map<void *, array_meta> M;
  return M;
}
} // namespace
...
template <typename T> const array_meta &ARRAY_META_DICT_AT(T *a) {
  if constexpr (std::is_pointer_v<T>) {
    ...
    return ARRAY_META_DICT().at(
        static_cast<void *>(const_cast<std::remove_cv_t<T> *>(a)));
  }
}

template <typename T> void ARRAY_META_DICT_SET(T *k, const array_meta &v) {
  std::lock_guard<std::mutex> g(meta_lock);
  ARRAY_META_DICT()[static_cast<void *>(k)] = v;
}
...
template <typename T> void deserialize(T &x, std::istream &s) {
  read_scalar(x, s);
}
...
// Struct-specific overloads follow below...
...
void add_line(const T &x, std::ostream &s, bool trailing_newline = true) {
  if constexpr (std::is_floating_point_v<T> || std::is_same_v<T, long double>) {
...
template <typename T> std::string serialize_array(T *arr) {
  const auto &m = ARRAY_META_DICT_AT(arr);
  std::stringstream s;
  add_line("# rank", s);
  add_line(m.rank, s);
  add_line("# size", s);
