    ...
    for state in sdfg.all_states():
        for node in state.nodes():
            if isinstance(node, dace.nodes.LibraryNode) and "deflatten" in node.label:
                ...
                deflatten_node = node
            if (
                isinstance(node, dace.nodes.LibraryNode)
                and "flatten" in node.label
                and "deflatten" not in node.label
            ):
                flatten_state = state
                flatten_node = node
                break
...
def rm_state_and_reroute(sdfg: dace.SDFG, state_names):
    for state in sdfg.all_states():
        if state.label in state_names:
            src_edge = (
                sdfg.in_edges(state)[0] if len(sdfg.in_edges(state)) == 1 else None
            )
            dst_edge = (
                sdfg.out_edges(state)[0] if len(sdfg.out_edges(state)) == 1 else None
            )
            ...
            if src_edge is not None and dst_edge is not None:
                ...
                for k, v in src_edge.data.assignments.items():
                    ...
                    if k in nassignments and v != nassignments[k]:
                        raise ValueError(
                            f"Duplicate assignment for key {k} in edge {src_edge}"
                        )
                ...
                sdfg.add_edge(
                    src_edge.src,
                    dst_edge.dst,
                    dace.InterstateEdge(assignments=nassignments),
                )
