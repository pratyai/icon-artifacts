...
def demote_symbol_to_scalar(sdfg: dace.SDFG, symbol_str: str):
    ...
    for e in sdfg.edges():
        if e.data is not None and e.data.assignments is not None:
            assert (
                len([(k, v) for k, v in e.data.assignments.items() if k == symbol_str])
                <= 1
            )
            if (
                len([(k, v) for k, v in e.data.assignments.items() if k == symbol_str])
                ...
                dst, assignment = [
                    (k, v) for k, v in e.data.assignments.items() if k == symbol_str
                ][0]
                # print(dst, assignment, type(assignment))
                if dst == symbol_str:
                    for cfg in (
                        e.dst.all_control_flow_regions()
                        if not isinstance(e.dst, dace.SDFGState)
                        else [e.dst]
                    ):
