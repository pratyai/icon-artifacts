...
def _add_copy_map(
    ...
    assert src_arr.shape == dst_arr.shape, (
        "Source and destination arrays must have the same shape."
    )
    ...
    map_entry, map_exit = state.add_map(
        name=f"copy_map_{src_arr_name}_to_{dst_arr_name}", ndrange=map_ranges
    )
...
def _lower_bidth_of_arrays_recursive(
    sdfg: dace.SDFG,
    array_names: Set[str],
    suffix: str,
    new_dtype: dace.dtypes.typeclass,
):
    if suffix == "int32":
        return
...
def _insert_lower_bidth_of_arrays_recursive(
...
def decrease_bitwidth_of_const_arrays(
    sdfg: dace.SDFG,
    array_names: Set[str],
    enable_uint16: bool = True,
    ...
    for name in array_names:
        if name in sdfg.arrays:
            ...
            if (
                sdfg.arrays[name].dtype == dace.int64
                or sdfg.arrays[name].dtype == dace.int32
            ):
    ...
    for node, graph in sdfg.all_nodes_recursive():
        if isinstance(node, dace.nodes.AccessNode) and node.data in array_names:
            if graph.in_degree(node) > 0 and any(
                ie.data is not None for ie in graph.in_edges(node)
            ):
    ...
    sdfgs_and_suffixes = [(copy_int32, "int32", dace.dtypes.int32)]
    if enable_uint16:
    ...
    assert len(new_sdfg.nodes()) == 3 and len(list(new_sdfg.all_states())) == 3, (
        f"Expected 3 nodes, got {len(new_sdfg.nodes())} and {len(list(new_sdfg.all_states()))} states got : {new_sdfg.nodes()}, {new_sdfg.all_states()}."
    )
    ...
    assert len(new_sdfg.out_edges(copy_in)) + len(new_sdfg.in_edges(copy_out)) == 2, (
        f"{new_sdfg.out_edges(copy_in)},{new_sdfg.in_edges(copy_out)}"
    )
        ...
        for suffix, dtype in suffix_and_dtypes:
            ...
            for name in array_names:
                ...
                if new_name not in new_sdfg.arrays:
                    ...
                    copy_desc.lifetime = dace.AllocationLifetime.Persistent
                    print(
                        f"Adding array {new_name} with dtype {copy_desc.dtype} to SDFG."
                    )
    ...
    check_if = ConditionalBlock(
        label="check_bitwidth_cond", sdfg=new_sdfg, parent=new_sdfg
    )
    check_cfg = ControlFlowRegion(
        label="check_bitwidth_cfg", sdfg=check_if.sdfg, parent=check_if
    )
    ...
    check_if.add_branch(
        condition=CodeBlock(code="bitwidth_check_done_sym == 0"), branch=check_cfg
    )
...
#include <stdio.h>
    ...
    check_state.add_edge(
        ...
        t,
        "_in_" + nproma_name,
        dace.Memlet.from_array(nproma_name, new_sdfg.arrays[nproma_name]),
    )
    ...
    check_state.add_edge(
        ...
        an,
        None,
        dace.Memlet.from_array("bitwidth_scalar", new_sdfg.arrays["bitwidth_scalar"]),
    )
    check_state.add_edge(
        t,
        "_out_bitwidth_check_done",
        an2,
        None,
        dace.Memlet.from_array(
            "bitwidth_check_done", new_sdfg.arrays["bitwidth_check_done"]
        ),
    )
    ...
    copy_bit = ConditionalBlock(label="copy_bit", sdfg=check_cfg.sdfg, parent=check_cfg)
    if enable_uint16:
        copy_cfg_i16 = ControlFlowRegion(
            label="copy_bit_cfg_i16", sdfg=copy_bit.sdfg, parent=copy_bit
        )
    copy_cfg_i32 = ControlFlowRegion(
        label="copy_bit_cfg_i32", sdfg=copy_bit.sdfg, parent=copy_bit
    )
    if enable_int64:
        copy_cfg_i64 = ControlFlowRegion(
            label="copy_bit_cfg_i64", sdfg=copy_bit.sdfg, parent=copy_bit
        )
    ...
    if enable_uint16 and enable_int64:
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 16"), branch=copy_cfg_i16
        )
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 32"), branch=copy_cfg_i32
        )
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 64"), branch=copy_cfg_i64
        )
    elif enable_uint16:
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 16"), branch=copy_cfg_i16
        ...
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 32 or bitwidth_sym == 64"),
            branch=copy_cfg_i32,
        )
    elif enable_int64:
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 32 or bitwidth_sym == 16"),
            branch=copy_cfg_i32,
        )
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 64"), branch=copy_cfg_i64
        ...
        copy_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym <= 64"), branch=copy_cfg_i32
        )
    check_cfg.add_node(copy_bit, is_start_block=False)
    check_cfg.add_edge(
        check_state,
        copy_bit,
        dace.InterstateEdge(assignments={"bitwidth_sym": "bitwidth_scalar"}),
    )
    ...
    for state, dst_dtype, suffix in ll:
        ...
        for arr_name in array_names:
            assert (
                new_sdfg.arrays[arr_name].storage
                == new_sdfg.arrays[f"{arr_name}_{suffix}"].storage
            )
            _add_copy_map(
                state,
                src_arr_name=arr_name,
    ...
    if enable_uint16:
        switch_cfg_i16 = ControlFlowRegion(
            label="switch_bit_cfg_i16", sdfg=switch_bit.sdfg, parent=switch_bit
        )
    switch_cfg_i32 = ControlFlowRegion(
        label="switch_bit_cfg_i32", sdfg=switch_bit.sdfg, parent=switch_bit
    )
    if enable_int64:
        switch_cfg_i64 = ControlFlowRegion(
            label="switch_bit_cfg_i64", sdfg=switch_bit.sdfg, parent=switch_bit
        )
    # Copy over only the needed body state
    if enable_uint16:
        switch_state_i16 = (
            copy_body_uint16  # dace.SDFGState(label="switch_i16", sdfg=switch_bit.sdfg)
        )
    switch_state_i32 = (
        copy_body_int32  # dace.SDFGState(label="switch_i32", sdfg=switch_bit.sdfg)
    )
    if enable_int64:
        switch_state_i64 = (
            copy_body_int64  # dace.SDFGState(label="switch_i64", sdfg=switch_bit.sdfg)
        )
    ...
    if enable_uint16 and enable_int64:
        switch_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 16"), branch=switch_cfg_i16
        )
        switch_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 32"), branch=switch_cfg_i32
        )
        switch_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 64"), branch=switch_cfg_i64
        )
    elif enable_uint16:
        switch_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym == 16"), branch=switch_cfg_i16
        ...
        switch_bit.add_branch(
            condition=CodeBlock(code="bitwidth_sym <= 64"), branch=switch_cfg_i32
        )
    ...
    def _check_data_desc(sdfg: dace.SDFG):
        for array_name, array in sdfg.arrays.items():
            if "uint16" in array_name:
                ...
                assert array.dtype == dace.uint16, (
                    f"Array {array_name} has dtype {array.dtype}, expected uint16."
                )
            elif "int32" in array_name:
                if array.dtype != dace.int32:
                    array.dtype = dace.int32
                assert array.dtype == dace.int32, (
                    f"Array {array_name} has dtype {array.dtype}, expected int32."
                )
            elif "int64" in array_name:
                if array.dtype != dace.int64:
                    array.dtype = dace.int64
                assert array.dtype == dace.int64, (
                    f"Array {array_name} has dtype {array.dtype}, expected int64."
                )
            else:
                assert array.dtype != dace.uint16, (
                    f"Array {array_name} has dtype {array.dtype}, expected not uint16."
                )
                assert array.dtype != dace.int64, (
                    f"Array {array_name} has dtype {array.dtype}, expected not int64."
                )
        for state in sdfg.all_states():
            for node in state.nodes():
                if isinstance(node, dace.nodes.NestedSDFG):
...
def force_decrease_bitwidth_of_nblk_arrays(
    sdfg: dace.SDFG, multi_val_array_names: Set[str], single_val_array_names: Set[str]
) -> dace.SDFG:
    ...
    assert len(top_level_nodes) == 4, (
        f"Expected 4 top level nodes, got {len(top_level_nodes)}: {top_level_nodes}"
    )
    ...
    repl_dict = {
        array_name: array_name + "_uint8" for array_name in multi_val_array_names
    }
    _insert_lower_bidth_of_arrays_recursive(
        body, multi_val_array_names, "uint8", dace.uint8
    )
    ...
    for array_name in single_val_array_names:
        ...
        for node, graph in body.all_nodes_recursive():
            ...
            if (
                ...
                and graph.parent_graph is not None
            ):
                if (
                    array_name in node.sdfg.arrays
                    and node.sdfg.arrays[array_name].transient is False
                ):
            ...
            if isinstance(edge.data, dace.InterstateEdge):
                if array_name in edge.data.assignments:
                    ...
                    if isinstance(val, str):
                        ...
                        raise ValueError(
                            f"Unexpected value {val} for assignment {array_name} in edge {edge}."
                        )
                    valstr = replace_array_with_constsym(valstr, array_name)
                    edge.data.assignments[array_name] = (
                        valstr if isinstance(val, str) else CodeBlock(code=valstr)
                    )
                for key, val in edge.data.assignments.items():
                    valstr = val if isinstance(val, str) else val.as_string
                    if array_name in valstr:
                        valstr = replace_array_with_constsym(valstr, array_name)
                        edge.data.assignments[key] = (
                            valstr if isinstance(val, str) else CodeBlock(code=valstr)
                        )
        ...
        for state in graph.all_states():
            for node in state.nodes():
                if isinstance(node, dace.nodes.NestedSDFG):
                    ...
                    missing_symbols = [
                        s for s in symbols if s not in node.symbol_mapping
                    ]
                    if missing_symbols:
                        # If symbols are missing, try to get them from the parent SDFG
                        assert isinstance(state, dace.SDFGState), (
                            "State should be an SDFGState."
                        )
                        parent_mapping = {
                            s: s
                            for s in missing_symbols
                            if s in state.symbols_defined_at(node)
                        }
...
""",
    ...
    lower_blk_bits_if = ConditionalBlock(
        label="lower_blk_cond", sdfg=new_sdfg, parent=new_sdfg
    )
    lower_blk_bits_cfg = ControlFlowRegion(
        label="lower_blk_cfg", sdfg=lower_blk_bits_if.sdfg, parent=lower_blk_bits_if
    )
    lower_blk_bits_state = dace.SDFGState(
        label="lower_blk", sdfg=lower_blk_bits_cfg.sdfg
    )
    lower_blk_bits_cfg.add_node(lower_blk_bits_state, is_start_block=True)
    lower_blk_bits_if.add_branch(
        condition=CodeBlock(code="nblk_lowering_done_sym == 0"),
        branch=lower_blk_bits_cfg,
    )
    ...
    lower_blk_bits_state.add_edge(
        t2,
        "_out_nblk_lowering_done",
        an,
        None,
        dace.Memlet.from_array(
            "nblk_lowering_done", new_sdfg.arrays["nblk_lowering_done"]
        ),
    )
...
def replace_array_with_constsym(text: str, array_name: str) -> str:
    ...
    while i < len(text):
        ...
        if (
            text[i : i + len(array_name)] == array_name
            and i + len(array_name) < len(text)
            and text[i + len(array_name)] == "["
        ):
