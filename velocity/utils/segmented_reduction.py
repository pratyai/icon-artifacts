...
def to_segmented_reduction(sdfg: dace.SDFG):
    all_maps = [
        (v, g)
        for v, g in sdfg.all_nodes_recursive()
        if isinstance(v, dace.sdfg.nodes.MapEntry)
    ]
    for map_entry, graph in all_maps:
        ...
        if len(all_nodes) == 3:
            ...
            tasklets = [
                node for node in all_nodes if isinstance(node, dace.sdfg.nodes.Tasklet)
            ]
            access_nodes = [
                node
                for node in all_nodes
            ...
            if len(lib_nodes) > 0 and len(tasklets) > 0 and len(access_nodes) > 0:
                ...
                if "reduce_sum_to_address" in lib_node.label:
                    ...
                    print(
                        f"Found reduce node {lib_node.label} in map {map_entry.label}"
                    )
...
""",
                    ...
                    graph.add_edge(
                        ...
                        "in_arr",
                        dace.memlet.Memlet(
                            data=input_edge.data.data,
                            subset=dace.subsets.Range(in_memlet_ranges),
                        ),
                    )
                    graph.add_edge(
                        srln,
                        ...
                        dace.memlet.Memlet(
                            data=output_edge.data.data,
                            subset=dace.subsets.Range(out_memlet_ranges),
                        ),
                    )

                    t1 = graph.add_tasklet(
                        name="t_segment_size",
                        inputs=set(),
                        outputs={"_out"},
                        code=f"_out = {segment_size}",
                    )
                    t2 = graph.add_tasklet(
                        name="t_batch_size",
                        inputs=set(),
                        outputs={"_out"},
                        code=f"_out = {batch_size}",
                    )
                    s1name, s1 = graph.sdfg.add_scalar(
                        name="segment_size",
                        dtype=dace.int32,
                    ...
                    s2name, s2 = graph.sdfg.add_scalar(
                        name="batch_size",
                        dtype=dace.int32,
                        storage=dace.StorageType.Register,
                        find_new_name=True,
                    ...
                    graph.add_edge(
                        ...
                        s1an,
                        None,
                        dace.memlet.Memlet(data=s1name, subset="0"),
                    )
                    graph.add_edge(
                        t2,
                        "_out",
                        s2an,
                        None,
                        dace.memlet.Memlet(data=s2name, subset="0"),
                    )
...
class ExpandGPU(pm.ExpandTransformation):
    ...
    def expansion(node: "SegmentedReduce", state: SDFGState, sdfg: SDFG):
        ...
        axes = (
            ...
            else [i for i in range(len(inedge.data.subset))]
        )
            ...
            init_state.add_mapped_tasklet(
                ...
                "__out = %s" % node.identity,
                {
                    "__out": dace.Memlet.simple(
                        "_out", ",".join(["_o%d" % i for i in osqdim])
                    )
                },
                external_edges=True,
            )
