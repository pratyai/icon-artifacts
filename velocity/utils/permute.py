...
def _replace_on_lines(
    sdfg: SDFG, old_name: str, new_name: str, permute_list: List[int]
):
                ...
                if e.data is not None:
                    ...
                    for k, v in e.data.assignments.items():
                        ...
                        for match in re.finditer(pattern, v):
                            assert match.group(1) == old_name, (
                                f"Expected {old_name}, got {match.group(1)}"
                            )
                            args_str = match.group(
                                2
                            )  # Get the arguments part as a string
                            old_pattern = (
                                re.escape(old_name)
                                + r"\["
                                + re.escape(args_str)
                            ...
                            assert len(args) == matchlen, (
                                f"Expected {matchlen} arguments, got {len(args), args, args_str, old_pattern, pattern, match}"
                            )
                            newargs = []
    ...
    for n, g in sdfg.all_nodes_recursive():
        if isinstance(n, dace.nodes.MapEntry) and n.map.params == [map_param]:
            ...
            g.add_edge(
                ...
                "_out1",
                s,
                "IN_" + "scan_reduce_stride",
                dace.Memlet.from_array(
                    "scan_reduce_stride", g.sdfg.arrays["scan_reduce_stride"]
                ),
            )
            red_node = LibNode(
                name=f"reduce_{type}",
            ...
            for ie in g.in_edges(l):
                g.add_edge(
                    ie.src, ie.src_conn, red_node, ie.dst_conn, copy.deepcopy(ie.data)
                )
            for oe in g.out_edges(l):
                g.add_edge(
                    red_node, oe.src_conn, oe.dst, oe.dst_conn, copy.deepcopy(oe.data)
                )
            g.remove_node(l)
            g.add_edge(
                s,
...
def permute_index(
    ...
    if Path(sdfg.name + "_perm_1.sdfgz").exists() and root.label == sdfg.label:
        ...
        if root == sdfg:
            ...
            for oldn, newn in name_map.items():
                ...
                MapDimShuffle.apply_to(
                    sdfg,
                    map_entry=map_entry,
                    options={"parameters": list(reversed(map_entry.map.params))},
                )
        ...
        for g in (
            sdfg.all_nodes_between(s1, s2)
            if sdfg == root
            else sdfg.all_control_flow_regions()
        ):
                    ...
                    if isinstance(n, dace.nodes.NestedSDFG):
                        ...
                        for ie in s.in_edges(n):
                            ...
                            if ie.dst_conn in name_map:
                                ...
                                ie.dst.add_in_connector(
                                    name_map[ie.dst_conn], force=True
                                )
                        ...
                        for oe in s.out_edges(n):
                            ...
                            if oe.src_conn in name_map:
                                ...
                                oe.src.add_out_connector(
                                    name_map[oe.src_conn], force=True
                                )
                ...
                for e in s.edges():
                    if e.data.data in name_map:
                        if e.dst_conn == "IN_" + e.data.data:
                            ...
                            e.dst.add_in_connector(
                                "IN_" + name_map[e.data.data], force=True
                            )
                        if e.src_conn == "OUT_" + e.data.data:
                            e.src_conn = "OUT_" + name_map[e.data.data]
                            e.src.remove_out_connector("OUT_" + e.data.data)
                            e.src.add_out_connector(
                                "OUT_" + name_map[e.data.data], force=True
                            )
                        oldn = e.data.data
    ...
    for old_name, new_name in orig_map.items():
        assert old_name in orig_permute_map or new_name in orig_permute_map, (
            f"Array {old_name} or {new_name} not in permute map"
        )
        perm = (
            orig_permute_map[old_name]
            if old_name in orig_permute_map
            else orig_permute_map[new_name]
        )
        ...
        if isinstance(n, dace.nodes.MapEntry):
            if inout_set is not None:
                inputs_and_outputs = set(
                    ...
                    [
                        e.dst.data
                        for e in g.out_edges(g.exit_node(n))
            ...
            if len(n.map.range) == len(local_permute_list):
                ...
                MapDimShuffle.apply_to(
                    g.sdfg, verify=True, map_entry=n, options={"parameters": new_params}
                )
