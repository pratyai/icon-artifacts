...
def map_for_44(sdfg: dace.SDFG):
    ...
    inner_nsdfg = [
        node
        for node in state_to_duplicate.nodes()
    ...
    outer_map_entry = [
        node
        for node in outermost_state.nodes()
        if isinstance(node, dace.nodes.MapEntry) and "_for_it_44" in node.map.params
    ][0]
    outer_nsdfg: dace.nodes.NestedSDFG = outermost_state.out_edges(outer_map_entry)[
        0
    ].dst
    out_edge = list(outermost_state.in_edges_by_connector(outer_nsdfg, "levmask"))[0]
    in_edge = list(
        outermost_state.in_edges_by_connector(
            outer_map_entry, out_edge.src_conn.replace("OUT_", "IN_")
        )
    )[0]
    inner_map_entry = [
        node
    ...
    state_to_duplicate.add_edge(
        an, None, inner_map_entry, f"IN_{conn_number}", dc(in_edge.data)
    )
    nsddfg = state_to_duplicate.out_edges(inner_map_entry)[0].dst
    assert nsddfg.add_in_connector("levmask")
    state_to_duplicate.add_edge(
        inner_map_entry, f"OUT_{conn_number}", nsddfg, "levmask", dc(out_edge.data)
    )
    # Add memlet symbol

    outer_nsdfg.symbol_mapping["nrdmax_jg"] = "nrdmax_jg"
    inner_nsdfg.sdfg.add_datadesc(
        "levmask", dc(state_to_duplicate.sdfg.arrays["levmask"])
    )
    # Add assignement
    innerstate_edge.data.assignments["_if_cond_24"] = outerstate_edge.data.assignments[
        "_if_cond_24"
    ]
    new_condition = "(_if_cond_24 == 1) and (_if_cond_25 == 1)"
    conditional_block.branches[0] = (
        dace.nodes.CodeBlock(new_condition),
        conditional_block.branches[0][1],
    )
...
def map_for_52(sdfg: dace.SDFG):
    ...
    outer_map_entry = [
        node
        for node in outermost_state.nodes()
        if isinstance(node, dace.nodes.MapEntry) and "_for_it_52" in node.map.params
    ][0]
    outer_nsdfg: dace.nodes.NestedSDFG = outermost_state.out_edges(outer_map_entry)[
        0
    ].dst
    ...
    state_to_duplicate.add_edge(
        inner_map_entry, f"OUT_{conn_number}", nsddfg, "levelmask", dc(out_edge.data)
    )
    # Add memlet symbol

    outer_nsdfg.symbol_mapping["nrdmax_jg"] = "nrdmax_jg"
    inner_nsdfg.sdfg.add_datadesc(
        "levelmask", dc(state_to_duplicate.sdfg.arrays["levelmask"])
    )
    # Add assignement
    innerstate_edge.data.assignments["_if_cond_28"] = outerstate_edge.data.assignments[
        "_if_cond_28"
    ]
