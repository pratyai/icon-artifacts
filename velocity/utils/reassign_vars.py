...
from utils.rename_on_if import (
    rename_on_if,
    rename_on_for,
    rename_on_map,
    rename_on_tasklet,
...
def replace(sdfg: dace.SDFG, start_block, old_symbol_str: str, new_symbol_str: str):
    ...
    """
    assert isinstance(start_block, dace.SDFGState) or isinstance(
        start_block, ControlFlowRegion
    ), "start_block must be a SDFGState or ControlFlowRegion."
    assert start_block in sdfg.bfs_nodes(source=start_block), (
        "start_block must be a valid starting point in the SDFG."
    )
...
def reassign_vars(sdfg: dace.SDFG):
    ...
    for node in sdfg.bfs_nodes(source=sdfg.start_block):
        ...
        if len(out_edges) == 1:
            ...
            assert isinstance(out_edge.data, dace.InterstateEdge), (
                f"Outgoing edge of node {node} is not an InterstateEdge."
            )
        ...
        if isinstance(node, dace.nodes.NestedSDFG):
            ...
            if missing_symbols:
                for missing_symbol in missing_symbols:
                    child_sdfg.add_symbol(
                        missing_symbol,
                        node.sdfg.symbols[missing_symbol]
                        if missing_symbol in node.sdfg.symbols
                        else dace.int32,
                    )
...
if __name__ == "__main__":
    sdfg = dace.SDFG.from_file(
        "codegen/gpu_velocity_no_nproma_if_prop_lvn_only_0_istep_1_stage1.sdfgz"
    )
    reassign_vars(sdfg)
    sdfg.save(
        "codegen/gpu_velocity_no_nproma_if_prop_lvn_only_0_istep_1_stage1_reassigned.sdfgz"
    )
    sdfg.validate()
