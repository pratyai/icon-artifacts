...
def _get_array_view_paths(
    sdfg: dace.SDFG, arrays_to_replace: Set[str]
) -> Dict[str, Set[str]]:
        ...
        for node in state.nodes():
            for node2 in state.nodes():
                ...
                if (
                    ...
                    and isinstance(node2, dace.nodes.AccessNode)
                    and isinstance(sdfg.arrays[node.data], dace.data.Data)
                    and isinstance(sdfg.arrays[node2.data], dace.data.View)
                    and node.data in arrays_to_replace
                ):
    ...
    for state in sdfg.all_states():
        for node in state.nodes():
            if isinstance(node, dace.nodes.NestedSDFG):
                ...
                _change_fp_type_recursive(
                    sdfg, src_fptype, dst_fptype, new_replacements
                )
...
def change_fptype(
    ...
    if arrays_to_replace is None:
        arrays_to_replace = {
            name for name, arr in sdfg.arrays.items() if arr.dtype == src_fptype
        }
    else:
    ...
    if cast_in_and_out_data is True:
        ...
        last_blocks = [
            node for node in sdfg.nodes() if sdfg.out_degree(node) == 0
        ]  # Only last block has no successors
        ...
        for original_arr_name, casted_arr_name in array_name_mapping.items():
            ...
            original_arr_desc = copy.deepcopy(
                arr
            )  # Transientness does not change for this array
            arr.transient = True  # Always transient due to copy-in
            original_arr_desc.dtype = src_fptype  # Change back to the original fp type
            sdfg.add_datadesc(
                name=original_arr_name, datadesc=original_arr_desc, find_new_name=False
            )
            src_dst_paris.add(
                ((casted_arr_name, arr), (original_arr_name, original_arr_desc))
            )
        ...
        def _add_copy_map(
            ...
            if isinstance(src_arr, dace.data.Array):
                ...
                map_entry, map_exit = state.add_map(
                    name=f"copy_map_{src_arr_name}_to_{dst_arr_name}",
                    ndrange=map_ranges,
                )
                ...
                state.add_edge(
                    ...
                    tasklet,
                    "in",
                    dace.Memlet(expr=f"{src_arr_name}[0]"),
                )
                state.add_edge(
                    tasklet,
                    "out",
                    dst_access,
                    None,
                    dace.Memlet(expr=f"{dst_arr_name}[0]"),
                )

        for (transient_arr_name, transient_arr), (
            nontransient_arr_name,
            nontransient_arr,
        ) in src_dst_paris:
