...
def change_flatten_lib_to_shallow_copy(
    sdfg: dace.SDFG,
    shallow_copy_used_structs: typing.List[str],
    deflatten_used_structs=typing.List[str],
) -> None:
    ...
    deflatten_destinations = {
        ie.dst for ie in deflatten_state.out_edges(deflatten_node)
    }
    ...
    for src in deflatten_sources:
        if deflatten_state.in_degree(src) == 0 and deflatten_state.out_degree(src) == 0:
            ...
            if (
                isinstance(ie_ie.src, dace.nodes.AccessNode)
                and ie_ie.src.data == "gpu_" + src.data
            ):
    ...
    shallow_deflatten_tasklet = deflatten_state.add_tasklet(
        name="shallow_deflatten",
        inputs=set(),
        outputs=set(),
        code=file_content,
    ...
    for name in rhs_set:
        if name in src_names:
            ...
            deflatten_state.add_edge(
                src, None, shallow_deflatten_tasklet, None, dace.Memlet()
            )
        else:
            a = deflatten_state.add_access(name)
            deflatten_state.add_edge(
                a, None, shallow_deflatten_tasklet, None, dace.Memlet()
            )
    ...
    shallow_copy_tasklet = flatten_state.add_tasklet(
        name="shallow_flatten",
        inputs=set(),
        outputs=set(),
        code=file_content,
        language=dace.Language.CPP,
    ...
    s = file_content
    lhs_set = {
        line.split("=")[0].strip() for line in s.strip().splitlines() if "=" in line
    }
    ...
    for _item in lhs_set:
        if _item not in src_names:
            ...
            flatten_state.add_edge(
                access_node, None, shallow_copy_tasklet, None, dace.Memlet()
            )
