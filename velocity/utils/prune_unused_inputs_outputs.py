        ...
        if isinstance(node, dace.SDFGState):
            for n in node.nodes():
                if isinstance(n, dace.nodes.MapEntry):
                    ...
                    if len(map_body_nodes) == 1 and isinstance(
                        map_body_nodes[0], dace.nodes.NestedSDFG
                    ):
                        ...
                        if len(inner_sdfg.nodes()) == 1 and isinstance(
                            inner_sdfg.nodes()[0], dace.SDFGState
                        ):
                            inner_state = inner_sdfg.nodes()[0]
                            maps_and_nested_sdfg = [
                                n
                                for n in inner_state.nodes()
                                if isinstance(n, dace.nodes.MapEntry)
                                or isinstance(n, dace.nodes.NestedSDFG)
                            ]
                            # print("2", maps_and_nested_sdfg)
                            if len(maps_and_nested_sdfg) == 2:
                                nested_sdfg = [
                                    n
                                    for n in maps_and_nested_sdfg
                                ...
                                srcs = list(
                                    set(
                                        [
                                            e.src
                                            for e in inner_state.in_edges(nested_sdfg)
                                        ]
                                    )
                                )
                                assert len(srcs) == 1
                                ...
                                dsts = list(
                                    set(
                                        [
                                            e.dst
                                            for e in inner_state.out_edges(nested_sdfg)
                                        ]
                                    )
                                )
                                assert len(dsts) == 1
                                        ...
                                        if ie.dst_conn == unused_input:
                                            ...
                                            if unused_input in inner_sdfg.arrays:
                                                ...
                                                for an in inner_state.nodes():
                                                    if (
                                                        isinstance(
                                                            an, dace.nodes.AccessNode
                                                        )
                                                        and an.data == unused_input
                                                    ):
                                                        used = True
                                                        break
                                                if not used:
                                            ...
                                            assert isinstance(
                                                ie.src, dace.nodes.MapEntry
                                            )
                                            ie.src.remove_out_connector(ie.src_conn)
                                            src_src_conn = ie.src_conn.replace(
                                                "OUT_", "IN_"
                                            )
                                            ...
                                            for iie in node.in_edges(ie.src):
                                                if (
                                                    iie.dst == ie.src
                                                    and iie.dst_conn == src_src_conn
                                                ):
                                                    node.remove_edge(iie)
                                                    if (
                                                        node.in_degree(iie.src) == 0
                                                        and node.out_degree(iie.src)
                                                        == 0
                                                    ):
                                    ...
                                    for oe in node.out_edges(nsdfg):
                                        if oe.src_conn == unused_output:
                                            ...
                                            for ooe in node.out_edges(oe.dst):
                                                if (
                                                    ooe.src == oe.dst
                                                    and ooe.src_conn == dst_dst_conn
                                                ):
                                                    node.remove_edge(ooe)
                                                    if (
                                                        node.in_degree(ooe.dst) == 0
                                                        and node.out_degree(ooe.dst)
                                                        == 0
                                                    ):
                    ...
                    if len(map_body_nodes) == 1 and isinstance(
                        ...
                        for name in all_inputs:
                            if (
                                ...
                                or "blk" in name
                                or "nproma" in name
                                or "cfl_clipping" in name
                                or "levmask" in name
                                or "maxvcfl" in name
                                or "owner_mask" in name
                                or "levelmask" in name
                            ):
                            ...
                            for ie in node.in_edges(nsdfg):
                                if ie.dst_conn == unused_input:
                                    ...
                                    if unused_input in inner_sdfg.arrays:
                                        ...
                                        for an in inner_state.nodes():
                                            if (
                                                isinstance(an, dace.nodes.AccessNode)
                                                and an.data == unused_input
                                            ):
                                                used = True
                                                break
