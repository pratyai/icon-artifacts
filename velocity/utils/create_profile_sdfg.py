...
def create_profile_sdfg(sdfg: dace.SDFG):
    id = 0
    new_first_state = sdfg.add_state_before(
        sdfg.start_state, "init_events", is_start_block=True, is_start_state=True
    )
    ...
    scl_desc = dace.data.Scalar(
        dtype=dace.int64, transient=True, storage=dace.StorageType.Register
    )
        ...
        if (
            ...
            for n in all_nodes:
                if isinstance(n, dace.nodes.NestedSDFG):
                    all_nodes_rec.extend(
                        [_n for _n, _g in n.sdfg.all_nodes_recursive()]
                    )
                else:
            ...
            num_tasklets = len(
                [n for n in all_nodes_rec if isinstance(n, dace.nodes.Tasklet)]
            )
            ...
            map_identifier = (
                node.label
                + "["
                + ", ".join(node.map.params)
                + f"][Num Tasklets: {num_tasklets}][Num IEdge Assignments: {num_indirect_assignments}](Profiler Id: {id})"
            )
            ...
            state.add_edge(
                ...
                stop_event_tasklet,
                "_in_tracker",
                dace.memlet.Memlet.from_array(dst.data, sdfg.arrays[dst.data]),
            )
            if len(dst_dsts) > 0:
                state.add_edge(
