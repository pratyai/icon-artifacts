...
def preprocess_tough_nut(sdfg: dace.SDFG):
    ...
    if node is None:
        print(
            "Warning: the if condition and index reassignment not found (possible lvn_only is False)"
        )
        return
    ...
    c = ConditionalBlock(
        label="extract_conditional", sdfg=parent_sdfg, parent=parent_graph
    )
    ...
    for n in after_state.nodes():
        if isinstance(n, dace.nodes.NestedSDFG):
            ...
            for e in node.edges():
                n.sdfg.add_edge(
                    node_map2[e.src], node_map2[e.dst], copy.deepcopy(e.data)
                )
            ...
            symbols = set(
                ...
                if k not in n.in_connectors and k not in n.out_connectors
            )
