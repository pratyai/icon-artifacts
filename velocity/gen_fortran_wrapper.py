...
def generate(all_params: dict[str, list[dict]], *,
    ...
    L.append("")
    L.append("    ! Fortran-side timers. Names carry a suffix (_fortran/_presync/")
    L.append("    ! _soprog) so extract_icon_timers.py's DACE_RE, which requires")
    L.append("    ! `_istep_<N> took`, ignores them and still sees only the core timer.")
    L.append("    INTEGER(c_int64_t) :: vt_t0, vt_t1, vt_rate, vt_p0, vt_p1, vt_w0, vt_w1")
    ...
    L.append("    CALL SYSTEM_CLOCK(vt_t0, vt_rate)")
    L.append("")
    ...
    for i, v in enumerate(VARIANTS):
        ...
        L.append(f"        )")
        L.append(f"      END IF")
        L.append(f"      CALL SYSTEM_CLOCK(vt_w0)")
        L.append(f"      !$ACC WAIT")
        L.append(f"      CALL SYSTEM_CLOCK(vt_w1)")
        L.append(f"      WRITE(0,'(A,I0,A)') 'Timer {v}_presync took ', &")
        L.append(f"        (vt_w1 - vt_w0) * 1000000_c_int64_t / vt_rate, ' us'")
        L.append(f"      CALL SYSTEM_CLOCK(vt_p0)")
        L.append(f"      CALL __program_{v}( &")
        L.append(f"        state_{s}, &")
        L.extend(emit_call_args(params, "        "))
        L.append(f"      )")
        L.append(f"      CALL SYSTEM_CLOCK(vt_p1)")
        L.append(f"      WRITE(0,'(A,I0,A)') 'Timer {v}_soprog took ', &")
        L.append(f"        (vt_p1 - vt_p0) * 1000000_c_int64_t / vt_rate, ' us'")
        L.append("")
    ...
    L.append("")
    L.append("    CALL SYSTEM_CLOCK(vt_t1)")
    L.append("    WRITE(vt_tag,'(A,I0,A,I0,A)') 'velocity_no_nproma_if_prop_lvn_only_', &")
    L.append("      MERGE(1,0,lvn_only), '_istep_', istep, '_fortran'")
    L.append("    WRITE(0,'(A,A,A,I0,A)') 'Timer ', TRIM(vt_tag), ' took ', &")
    L.append("      (vt_t1 - vt_t0) * 1000000_c_int64_t / vt_rate, ' us'")
    L.append("")
